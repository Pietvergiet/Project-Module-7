...
def timer(graphs):
	...
	print("voor", len(graph4))
	both = time.time()
	graph4 = Pre.checkLength(graph4)
	print("tussen", len(graph4))
...
def main():
	graph = G.loadGraphs("week2/cubes4.grl")
	print("Test with all isomorphic graphs: ")
	timer(graph)

	graph1 = G.loadGraphs("test/cubes4_VeE.grl")
	print("Test with one graph with less vertices/egdes: ")
	timer(graph1)

	graph2 = G.loadGraphs("test/cubes4_con.grl")
	print("Test with one disconnected graph: ")
	timer(graph2)

	graph3 = G.loadGraphs("test/cubes4_both.grl")
