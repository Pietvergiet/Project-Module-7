...
\begin{algorithm}[H]
	...
	\For{Alle vertices v in de lijst}
		\For{Alle vertices k van de kleur van de node}
			Nieuwekleuren = []\\
			\If{De kleuren van de neighbors van v ongelijk zijn aan de neigbors van k}
				\For{Alle nieuwe kleuren}
					\If{De neighbors van k gelijk zijn aan die van een van de nieuwe kleuren}
...
\begin{algorithm}[H]
	...
	\For{Alle graven g in dupVertices}
		\For{Alle vertices v van g}
			...
			\If{De kleuren van de twee aangepaste grafen gelijk zijn}
				\If{Er geen grafen zonder unieke kleuring meer zijn}
					\Return de nieuwe lijsten
				\Else 
					Pas individual refinement opnieuw toe
					\If{Er geen isomorfisme gevonden is}
